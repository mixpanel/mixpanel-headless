...
- [x] T087 [P] ~~Update `mixpanel-plugin/help.py` so `python help.py Replay`, `python help.py ReplayBundle`, etc. return the documented signature + docstring + related types.~~ SUPERSEDED by Plan 047 (`context/help-integration-plan.md`): the script is replaced by the library's built-in `mp help` / `mp.help()`, which covers `Replay`, `ReplayBundle`, and every other export from the `__all__` inventory.
