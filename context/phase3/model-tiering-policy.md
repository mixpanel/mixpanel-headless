...
| B2 validators (`validation.py`, `user_validators`) translation + Layer-3 tests | **opus** | Mechanical rule transcription, fully locked by validation-error vectors + code-registry equality |
...
| B5 services (discovery, live_query, replays, rrweb analyzer) | **opus** | Wire/parse-vector-locked; rrweb has golden files |
| B6 workspace facade (205 members) | **opus** | The bulk of the queue; mechanical delegation + options mapping, delegation-equivalence PBT locks it |
...
2. **Escalation on failure**: a volume-tier (opus) task that misses its
...
3. **Review is always cross-tier**: every opus translation gets Fable adversarial
...
5. ~~Sonnet intro pricing~~ — void; Sonnet removed from the program (see REVISION below).
...
## REVISION 2026-08-15 (user directive)

Sonnet is REMOVED from the tiering program: the harness "sonnet" alias resolved to
Sonnet 4.5 (not Sonnet 5) on this Vertex deployment, observed live on task
b2:m1-validation-a. Volume tier is now **Opus 5 (1M context) exclusively** — the "opus"
alias is pinned to `claude-opus-5[1m]` via ANTHROPIC_DEFAULT_OPUS_MODEL in
.claude/settings.local.json (probe-verified 2026-08-15: subagent reports
id=claude-opus-5[1m]; note the base ID prefix is required — the bare string
`opus-5[1m]` is not parsed as a variant and errors). Two tiers only:
fable (everything load-bearing) + opus (B2/B3/B5/B6 volume translation). Escalation
rule unchanged: volume-tier attempt-1 miss retries on fable.
